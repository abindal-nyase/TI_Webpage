...
// ── DISC DATA (from JSON) ──────────────────────────────────────────────────────
const DISC_DATA = SIM_DATA.discs.map(d => ({
  radius:    d.radius,
...
const N = DISC_DATA.length   // 7

// ── VISUAL GEOMETRY ───────────────────────────────────────────────────────────
const FACE_H     = Math.round(DISC_DATA[0].thickness * DISCS.blenderScale)
const SIDE_NET_H = FACE_H
const SIDE_EL_H  = FACE_H / 2 + SIDE_NET_H

// ── COORDINATE MAPPING ────────────────────────────────────────────────────────
const BL_OX = DISC_DATA[0].initCX
const BL_OZ = DISC_DATA[0].initCZ

const PAD           = 10
const maxInitCZ     = Math.max(...DISC_DATA.map(d => d.initCZ))
const blenderRangeZ = maxInitCZ - BL_OZ

const ORIGIN_PX = PAD + Math.round(DISC_DATA[0].radius * DISCS.blenderScale)
const ORIGIN_PZ = PAD + Math.round(blenderRangeZ * DISCS.blenderScale) + Math.round(FACE_H / 2)

// ── TOWER CONTAINER DIMENSIONS ────────────────────────────────────────────────
const maxDiscPxWidth = Math.max(...DISC_DATA.map(d => d.pxWidth))
...
const disc0RightPx    = PAD + DISC_DATA[0].pxWidth
const WOBBLE_ORIG_PCT = Math.round((disc0RightPx / TOWER_W) * 100)

// ── GREEN STACK DATA ──────────────────────────────────────────────────────────
const GREEN_DISC_DATA = DISC_DATA.map((redDisc, i) => {
  const pxWidth  = DISC_DATA[N - 1 - i].pxWidth
...
// ── ITEMS ─────────────────────────────────────────────────────────────────────
const ITEMS = [
  { category: 'Communication',      text: 'Poor team coordination causes uncertainty and schedule loss.' },
  { category: 'Fee Accuracy',       text: 'Low initial proposal hides scope gaps — add-service costs follow.' },
  { category: 'Plan Check',         text: "Slow approvals because drawings don't anticipate city requirements." },
  { category: 'Field Coordination', text: 'Generic details cause conflicts and RFIs during construction.' },
  { category: 'Design Quality',     text: 'Innovative ideas compromised — engineer cannot support the vision.' },
  { category: 'Early Clarity',      text: 'Critical decisions made before structural constraints are understood.' },
  { category: 'Building Knowledge', text: 'Structural system never studied — the whole project rests on a false premise.' },
]
...
const COLORS = [
  { face: '#2c2020', rim: '#1c1414' },
  { face: '#3e2424', rim: '#2a1818' },
  { face: '#532a2a', rim: '#3b1e1e' },
  { face: '#6a2e2e', rim: '#4c2020' },
  { face: '#863636', rim: '#602828' },
  { face: '#a03c3c', rim: '#782c2c' },
  { face: '#b84040', rim: '#8a2e2e' },
]

const GREEN_COLORS = [
  { face: '#172e22', rim: '#0e1d16' },
  { face: '#1d3d2b', rim: '#12271b' },
  { face: '#235233', rim: '#163320' },
  { face: '#29663d', rim: '#1a4227' },
  { face: '#2f7a47', rim: '#1e4f2d' },
  { face: '#379956', rim: '#235e35' },
  { face: '#3a9966', rim: '#286644' },
]
...
export default function ProblemsSolutionsNewOptions() {
  ...
  const fall         = phase >= N
  const greenReveal  = phase >= N + 1
  const droppedGreen = Math.min(N, Math.max(0, phase - (N + 1)))
  ...
  useEffect(() => {
    ...
    function tick(now) {
      ...
      const styles = DISC_DATA.map(disc => ({
  ...
  const redTopI      = dropped > 0 ? dropped - 1 : 0
  const redBarW      = dropped > 0 ? Math.round(DISC_DATA[redTopI].pxWidth * DISCS.towerScale) : 0
  const riskRedPct   = dropped > 0 ? Math.round(dropped / N * 100) : 0
  const riskRedLabel = dropped > 0 ? ITEMS[redTopI].category : ''

  // B — Green horizontal risk bar (width = last-placed green disc width, scaled)
  const greenTopI      = droppedGreen > 0 ? droppedGreen - 1 : 0
  const greenBarW      = droppedGreen > 0 ? Math.round(GREEN_DISC_DATA[greenTopI].pxWidth * DISCS.towerScale) : 0
  const riskGreenPct   = droppedGreen > 0 ? Math.round(droppedGreen / N * 100) : 0
  const riskGreenLabel = droppedGreen > 0 ? ITEMS[greenTopI].category : ''

  // C — Red vertical project-value bar (height = scaled distance from tower bottom to top disc face)
  const redValueH = dropped > 0
    ? Math.round((TOWER_H - toScreenTop(DISC_DATA[dropped - 1].initCZ)) * DISCS.towerScale)
    : 0
  ...
  const curveEndColor2 = 'rgba(240,236,227,0.75)'
  ...
  return (
    <div className={s.root}>
              ...
              <div className={s.towerWithBar}>
                ...
                <div className={s.valueBarWrap}>
                  ...
                  <div className={s.valueBar} style={{
                    ...
                    background: fall ? '#b84040' : 'rgba(176,168,154,0.55)',
                    transition: fall
                      ? 'none'
                ...
                <div className={s.towerOuter} style={{ width: TOWER_PX_W, height: TOWER_PX_H }}>
                  <div className={s.towerInner} style={{ transform: `scale(${DISCS.towerScale})` }}>
                    <div
                      className={s.tower}
                      style={{ width: TOWER_W, height: TOWER_H, transformOrigin: `${WOBBLE_ORIG_PCT}% 97%` }}
                    >
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              zIndex:          i + 1,
                              '--f': COLORS[i].face, '--r': COLORS[i].rim,
                              ...discVars,
                            }}
                          >
              ...
              <div className={[s.barArea, greenReveal && s.barHidden].filter(Boolean).join(' ')}>
                ...
                <div className={s.riskBarWrap}>
                  <div className={s.riskBarLabel}>
                    ...
                    <span className={s.riskBarPct}>{riskRedPct}%</span>
                    {riskRedLabel && <span className={s.riskBarCat} style={{ opacity: !greenReveal ? 1 : 0, transition: 'opacity 0.6s ease' }}>{riskRedLabel}</span>}
                  </div>
                  <div className={s.riskBar} style={{
                    width:      redBarW,
                    height:     DISCS.riskBarH,
                    background: 'linear-gradient(to right, #b84040, #c85050)',
              ...
              <div className={s.towerWithBar}>
                ...
                <div className={s.valueBarWrap} style={{ opacity: greenReveal ? 1 : 0, transition: 'opacity 0.3s ease' }}>
                  ...
                  <div className={s.valueBar} style={{
                    width:      DISCS.valueBarW,
                    height:     greenValueH,
                    background: droppedGreen === N ? '#3a9966' : 'rgba(176,168,154,0.55)',
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              transformOrigin: `50% ${FACE_H / 2}px`,
                              zIndex:          i + 1,
                              '--f': GREEN_COLORS[i].face, '--r': GREEN_COLORS[i].rim,
                              ...discVars,
                            }}
                          >
              ...
              <div className={[s.barArea, !greenReveal && s.barHidden].filter(Boolean).join(' ')}>
                ...
                <div className={s.riskBarWrap}>
                  <div className={s.riskBarLabel}>
                    ...
                    <span className={s.riskBarPct}>{riskGreenPct}%</span>
                    {riskGreenLabel && <span className={s.riskBarCat} style={{ opacity: phase < PHASES - 1 ? 1 : 0, transition: 'opacity 0.6s ease' }}>{riskGreenLabel}</span>}
                  </div>
                  <div className={s.riskBar} style={{
                    width:      greenBarW,
                    height:     DISCS.riskBarH,
                    background: 'linear-gradient(to right, #3a9966, #4ab87a)',
                ...
                >
                  ...
                  <div className={s.cardBody}>
                    ...
                    <div className={s.negSection}>
                      ...
                      <p className={s.cardTxt}>{item.text}</p>
                    </div>
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          OPTION 2 — BARS & BUBBLES (full-width, vertically spread)
      ══════════════════════════════════════════════════════════════════════ */}
        ...
        <div className={s.scene2}>
          <div className={s.chartArea2} ref={chartRef2}
              ...
              >
                <defs>
                  <linearGradient id="curveGrad2" gradientUnits="userSpaceOnUse" x1="0" y1="0" x2={cw} y2="0">
                    ...
                    <stop offset={`${gradTransitionPct2}%`}   stopColor="#c85050" />
                    <stop offset={`${Math.min(100, gradTransitionPct2 + 5)}%`} stopColor="#4ab87a" />
                    <stop offset="100%"                       stopColor="#4ab87a" />
                  </linearGradient>
            ...
            {cw > 0 && ITEMS.map((item, i) => {
              ...
              const glow = `0 0 ${BARS.glowRadius}px rgba(184,64,64,0.75)`
                  ...
                  {on && (
                    <div
                      ...
                      style={{
                        ...
                        background: 'radial-gradient(circle, #c85050, #7a2e2e)',
                        boxShadow: live ? glow : 'none',
                      }}
                    >
                      <span className={s.bubbleCat2}>{item.category}</span>
                      <span className={s.bubbleTxt2}>{item.text}</span>
                    </div>
                  )}
                  <div
                    className={s.bar2}
                    style={{
                      width: BARS.barWidth,
                      height: on ? barH : 0,
                      background: 'linear-gradient(to bottom, #c85050, #6a2e2e)',
                      boxShadow: live ? glow : 'none',
            ...
            {cw > 0 && ITEMS.map((item, i) => {
              ...
              const glow = `0 0 ${BARS.glowRadius}px rgba(58,153,102,0.75)`
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      height: BARS.bubbleDiameter,
                      borderRadius: BARS.bubbleDiameter / 2,
                      background: 'radial-gradient(circle, #4ab87a, #1d5c3a)',
                      boxShadow: live ? glow : 'none',
                    }}
                  >
                    <span className={s.bubbleCat2}>{item.category}</span>
                    <span className={s.bubbleTxt2}>{SOLUTION_TEXTS[i]}</span>
                  </div>
                  <div
                    className={s.bar2}
                    style={{
                      width: BARS.barWidth,
                      height: barH,
                      background: 'linear-gradient(to bottom, #4ab87a, #1a4a33)',
                      boxShadow: live ? glow : 'none',
                      transition: `height ${BARS.barAnimDuration}s cubic-bezier(0.18,1.18,0.38,1)`,
                    }}
                  />
